...
class Product {
  ...
  static async publish(id) {
    try {
      ...
      const result = await dynamodbService.updateItem(id, {
        status: 'active',
  ...
  static async archive(id) {
    try {
      logger.info(`Archiving product: ${id}`);
   ...
   * @param {string} id - Product ID
   * @param {Object} statData - Statistics to update
   * @returns {Promise<Object>} Updated product object
   */
  static async updateStats(id, statData) {
    try {
      logger.info(`Updating stats for product: ${id}`);

      const product = await dynamodbService.getItem(id, 'productId');
      if (!product.success) {
      ...
      logger.info(`Stats updated for product: ${id}`);
      return result.data;
    } catch (error) {
      logger.error(`Error updating product stats: ${error.message}`);
      throw error;
    }
  }
   ...
   * @returns {Promise<Object>} Updated product object
   */
  static async addTeamMember(productId, userId) {
    try {
      logger.info(`Adding team member ${userId} to product ${productId}`);

      const product = await dynamodbService.getItem(productId, 'productId');
      if (!product.success) {
        throw new Error('Product not found');
      }

      const team = product.data.team || [];
      if (!team.includes(userId)) {
        team.push(userId);

        const result = await dynamodbService.updateItem(productId, {
          team: team,
          updatedAt: new Date().toISOString(),
        }, 'productId');

        logger.info(`Team member added to product: ${productId}`);
        return result.data;
      }
   ...
   * @returns {Promise<Object>} Updated product object
   */
  static async removeTeamMember(productId, userId) {
    try {
      logger.info(`Removing team member ${userId} from product ${productId}`);
      ...
      const team = (product.data.team || []).filter(id => id !== userId);
      ...
      logger.info(`Team member removed from product: ${productId}`);
      return result.data;
    } catch (error) {
  ...
  /**
   * Get trending products (sorted by stats)
   * @param {number} limit - Maximum number of products to return
  ...
  static async getTrending(limit = 20) {
    try {
      ...
      const result = await dynamodbService.scan({ Limit: 100 });
      if (!result.success) {
        throw new Error('Failed to fetch trending products');
      }

      // Sort by stats (downloads + stars + views)
      const sorted = result.data.sort((a, b) => {
        const scoreA = (a.stats?.downloads || 0) + (a.stats?.stars || 0) + (a.stats?.views || 0);
        const scoreB = (b.stats?.downloads || 0) + (b.stats?.stars || 0) + (b.stats?.views || 0);
